...
function matches(tables: Table, term: string, pipe: PipeTransform) {
    return tables.state_id.toLowerCase().includes(term.toLowerCase())
...
export class AdvancedService {
    ...
    constructor(private pipe: DecimalPipe,private http: HttpClient) {
        ...
        this.getAllCities(5,this.page).subscribe( data => {
            this._tables$.next(data.data.data);
            this._total$.next(data.data.total);
            this.startIndex = data.data.from;
            this.endIndex = data.data.to;
    ...
    getAllCities(perpage, pageno) {
        return this.http.get<any>(environment.api_base_url+`/api/cities?perPage=${perpage}&page=${pageno}`);
    }
}
